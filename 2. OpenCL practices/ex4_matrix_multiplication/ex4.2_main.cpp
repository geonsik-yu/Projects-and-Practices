/*
 Title: OpenCL Matrix-Matrix Multiplication Example 2
 References:
...
int main(int argc, char** argv){
	...
	// 3. Program ----------------------------------------------------------
	program = CreateProgram(context, device, "ex4.2_matrix_multiplication.cl");
	if (program == NULL){
		Cleanup(context, commandQueue, program, kernel, memObjects);
		return 1;
	}
	...
	size_t globalWorkSize = (size_t) n;
	size_t localWorkSize = (size_t) n / 32;
	//size_t localWorkSize = (size_t) n / 4; //for "problem_1.txt"

	// Iris Pro (my cute test device) has max CU number of 40, but we use 32 instead to divide the whole workload evenly.
