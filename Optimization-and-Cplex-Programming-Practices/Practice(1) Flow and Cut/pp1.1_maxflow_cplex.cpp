/*
 Title: Cplex, Maximum Flow Problem.
 Description: This c++ source code determines the maximum flow value of the given flow network.
 Note:
...
IloModel read_lp(int& n, int& m, IloEnv env){
	...
	while (getline(fin, read_str)){
		...
		capacity = atof(line_tokens.at(2).c_str());
		cout << capacity << endl;
