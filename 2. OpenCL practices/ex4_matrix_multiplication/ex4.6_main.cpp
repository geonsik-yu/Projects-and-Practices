/*
 Title: OpenCL Matrix-Matrix Multiplication Example 6
 References:
 ...
 Note:
	...
	- 3rd optimization
		: number of columns loaded in local memory per each iteration (in kernel) increased to 4. Accordingly, the number of barrier usage (synchronization) is decreased.
...
int main(int argc, char** argv){
	...
	// 3. Program ----------------------------------------------------------
	program = CreateProgram(context, device, "ex4.6_matrix_multiplication.cl");
	if (program == NULL){
		Cleanup(context, commandQueue, program, kernel, memObjects);
		return 1;
	}
