/*
 Title: OpenCL Hello World! (Element-wise square)
 References:
	[1] "Hello World!". Developer.apple.com. N.p., 2017. Web. 1 July 2017.
 Note:
...
// Include the header file generated by Xcode.
// This header file contains the kernel block declaration.
#include "ex1_square.cl.h"
...
static int validate(cl_float* input, cl_float* output) {
	...
	for (i = 0; i < NUM_VALUES; i++) {
		...
		if ( output[i] != (input[i] * input[i]) ) {
			...
			fprintf(stdout,
					"       Result %1.4f, expected %1.4f\n", output[i],
					input[i] * input[i]);
			fflush(stdout);
			return 0;
		}
		fprintf(stdout,
				"       Result %1.4f, input %1.4f\n", output[i],
				input[i]);
...
int main (int argc, const char * argv[]) {
	...
	char name[128];
 	...
 	// Step 1 ------------------------------------------------------------------------
	// : Dispatch Queue
	// First, try to obtain a dispatch queue that can send work to the GPU in our system.
	...
	// The output array is not initalized; we're going to fill it up when we execute our kernel.
	void* mem_out = gcl_malloc(sizeof(cl_float) * NUM_VALUES, NULL, CL_MEM_WRITE_ONLY);
 

	// Step 3 ------------------------------------------------------------------------
	// : Dispatch Kernel Blocks.
	// Dispatch the kernel block using one of the dispatch_ commands and the queue created earlier.
	dispatch_sync(queue, ^{
		...
		size_t wgs;
		gcl_get_kernel_block_workgroup_info(my_square_kernel,
		...
		cl_ndrange range = {
			...
			{wgs, 0, 0}			// The local size of each workgroup.  This
								...
								// since ( global size / local size ) yields
		...
		my_square_kernel(&range,(cl_float*)mem_in, (cl_float*)mem_out);
