/*
 Title: OpenCL Matrix-Matrix Multiplication Example 5
 References:
 ...
 Note:
	...
	- 2nd optimization
		: use local memory for each column of matrix b to reduce usage of global memory.
		: local memory barrier is used for synchronization.
	- This example assumes that the input values have only one significant decimal digit to the right of the decimal point (therefore the result values have two significant digits to the right of the decimal point). Thus, we simply multiply the values by 10 to use integer data type and avoid floating number arithmetic error.
*/
...
static float ERROR_BOUND = 0.0000001;

// Set-up Function Declarations.
cl_context CreateContext();
cl_command_queue CreateCommandQueue(cl_context context, cl_device_id *device);
cl_program CreateProgram(cl_context context, cl_device_id device, const char* fileName);
bool CreateMemObjects(cl_context context, cl_mem memObjects[3], int *a, int *b, size_t n, size_t p, size_t m);
void Validation(int* correct_result, int* result, size_t n, size_t m);
...
void PrintMatrix(size_t Nrow, size_t Ncol, int* matrix);
...
int main(int argc, char** argv){
	...
	// 3. Program ----------------------------------------------------------
	program = CreateProgram(context, device, "ex4.5_matrix_multiplication.cl");
	if (program == NULL){
		Cleanup(context, commandQueue, program, kernel, memObjects);
		return 1;
	}
	...
	int* a = new int[padded_n*p];//[DIM_N][DIM_P];
	int* b = new int[p*m];//[DIM_P][DIM_M];
	int* result = new int[padded_n*m];//[DIM_N][DIM_M];
	...
	for(size_t i = 0; i < n; i++){
		...
		for(size_t j = 0; j < p; j++){
			ss >> buf; a[i*p+j] = (int)(atof(buf.c_str())*10);
		}
	}
	...
	for(size_t i = 0; i < p; i++){
		...
		for(size_t j = 0; j < m; j++){
			ss >> buf; b[i*m+j] = (int)(atof(buf.c_str())*10);
		}
	}
	//PrintMatrix(n, p, a);
	//PrintMatrix(p, m, b);
	...
	//size_t localWorkSize[1] = {(size_t) n / 32};
	...
	errNum = clEnqueueNDRangeKernel(commandQueue, kernel, 1, NULL, &globalWorkSize, &localWorkSize, 0, NULL, &prof_event);
	errNum = clEnqueueReadBuffer(commandQueue, memObjects[2], CL_TRUE, 0, padded_n*m*sizeof(int), result, 0, NULL, &prof_event);
	if (errNum != CL_SUCCESS){
	...
	int* correct_result = new int[n*m];
	for(int i = 0; i < n; i++){
		getline(fin, read_str);
		std::stringstream ss(read_str);
		for(int j = 0; j < m; j++){
			ss >> buf; correct_result[i*m+j] = (int)(atof(buf.c_str())*100);
		}
	}
...
bool CreateMemObjects(cl_context context, cl_mem memObjects[3], int *a, int *b, size_t n, size_t p, size_t m){
	memObjects[0] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int)*n*p, a, NULL);
	memObjects[1] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int)*p*m, b, NULL);
	memObjects[2] = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int)*n*m, NULL, NULL);
...
void Validation(int* correct_result, int* result, size_t n, size_t m){
	...
	for(size_t i = 0; i < n*m; i++){
		if( abs(correct_result[i] - result[i]) > ERROR_BOUND ){
			flag = false;// break;
			std::cout << std::setprecision(20) << "error: "<< abs( result[i] - correct_result[i] )/100 << std::endl;
		}
	}
	if(flag==true){
...
void PrintMatrix(size_t Nrow, size_t Ncol, int* matrix){
	...
	for(size_t i = 0; i < Nrow; i++){
		for(size_t j = 0; j < Ncol; j++)
			std::cout << std::setprecision(7) << (float)(matrix[(Ncol*i)+j])/100.f  << " ";
		std::cout << std::endl;
	}
	std::cout << std::endl;
};
